...
pub async fn untriaged(db: &Db, limit: i64) -> Result<Vec<RawItem>> {
    ...
    let rows = crate::sql(format!(
        "SELECT {cols} FROM (
         ...
         -- Weighted edition round-robin: Simplified Chinese gets two slots per
         -- round because it is the primary product; Traditional Chinese,
         -- English, Japanese and Korean each get one. Without this, a busy
         -- primary queue can delay a one-source Korean edition by hours.
         ORDER BY CASE WHEN q.edition = 'zh' THEN (q.lang_rank + 1) / 2 ELSE q.lang_rank END,
                  CASE q.edition
                    WHEN 'zh' THEN 0 WHEN 'zh-hant' THEN 1 WHEN 'en' THEN 2
                    WHEN 'ja' THEN 3 WHEN 'ko' THEN 4 ELSE 5
                  END,
